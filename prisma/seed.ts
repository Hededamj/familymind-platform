...
/**
 * Helper for models without a unique field.
 * Checks if a record exists via findFirst; creates only if not found.
 */
...
async function main() {
  ...
  // -- Re-engagement Tiers --
  // These reference emailTemplates[4..7] (the reengagement_tier1..4 templates)
  await Promise.all([
  ...
  const q1 = await seedIfNotExists(
  ...
  const q2 = await seedIfNotExists(
  ...
  const q3 = await seedIfNotExists(
  ...
  const q4 = await seedIfNotExists(
  ...
  const q5 = await seedIfNotExists(
  ...
  // Create some tags first for mapping (ContentTag has @unique slug)
  ...
  // Q2 options (challenges, mapped to tags) — skip if question already has options
  const q2OptionsExist = await prisma.onboardingOption.findFirst({ where: { questionId: q2.id } })
  if (!q2OptionsExist) {
    await Promise.all([
  ...
  // Q3 options (interests) — skip if question already has options
  const q3OptionsExist = await prisma.onboardingOption.findFirst({ where: { questionId: q3.id } })
  if (!q3OptionsExist) {
    await Promise.all([
  ...
  // Q4 options (time preferences) — skip if question already has options
  const q4OptionsExist = await prisma.onboardingOption.findFirst({ where: { questionId: q4.id } })
  if (!q4OptionsExist) {
    await Promise.all([
  ...
  // Q5 options (goals) — skip if question already has options
  const q5OptionsExist = await prisma.onboardingOption.findFirst({ where: { questionId: q5.id } })
  if (!q5OptionsExist) {
    await Promise.all([
  ...
  // -- Test Journey --
  // If journey slug already exists, load the existing journey; otherwise create everything
  const existingJourney = await prisma.journey.findUnique({ where: { slug: 'bedre-sovnrutiner' } })

  const journey = existingJourney ?? await prisma.journey.create({
    data: {
  ...
  // Only create phases, days, and actions if the journey was just created
  if (!existingJourney) {
  ...
  await seedIfNotExists(
    () => prisma.recommendationRule.findFirst({ where: { name: 'Søvnudfordringer → Søvnrejse' } }),
    () => prisma.recommendationRule.create({
      data: {
